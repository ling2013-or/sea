...
class OrderModel extends Model
{
     ...
     */
    public function calcBuyList($id,$num)
    {
        $model = M('Goods');
        $map = [];
        $map['id'] = $id;
        $map['status'] = 1;
        $goods = $model->field('*')->where($map)->find();
        if (empty($goods['id'])) {
            $this->code = 51221;
        ...
        $return = $goods;
        $price = abs($goods['price']*$num);
        if ($goods['goods_type'] == 1) {
            if ($goods['gids']) {
                $array = explode(',', $goods['gids']);
                $condition = [];
                $condition['id'] = array('in', $array);
                $return = $model->field('*')->where($condition)->select();
            }
        }

        return [$goods['goods_type'], $price, $return];
     ...
     * @return bool|int
     */
    public function createOrder($cart_id, $uid, $address_id = 0, $num = 1)
    {
        // 获取用户基本资料
        ...
        list($order_type, $goods_total, $goods_info) = $this->calcBuyList($cart_id,$num);
        if (empty($cart_list)) {
            $this->code = 51212;
        ...
        //收货人省份ID
        $order['province_id'] = $address_info['province_id'];
        //收货人城市ID
        ...
        foreach ($goods_info as $goods) {
            if ($goods['status'] != 1) {
                $this->code = 51216;
                $this->error = '部分商品已经下架或库存不足，请重新选择';
                return false;
            }

            $order_goods[] = array(
                'order_id' => $order_id,    // 订单号
                'goods_id' => $goods['id'], // 产品ID
                'goods_price' => $goods['price'], // 产品价格
                'goods_num' => abs($num), // 产品数量
                'goods_name' => $goods['name'], // 产品名称
                'goods_cover' => $goods['picture'], // 商品图片
                'zone_id' => $goods['goods_image'],     // 分区ID
