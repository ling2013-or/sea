                    ...
                    <div class="operate-body">
                        ...
                        <div class="pull-right search-form form-inline">
                            <form action="" onsubmit="return false">
                                <input type="text" name="title" class="form-control search-input" value="<?php echo I('title');?>" placeholder="请输入监控标题">
                                <input type="text" name="camera_id" class="form-control search-input" value="<?php echo I('camera_id');?>" placeholder="请输入监控ID">
                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td><?php echo ($vo["ztitle"]); ?></td>
                                        <td><?php echo ($vo["camera_id"]); ?></td>
                                        <td><?php echo ($vo["server_ip"]); ?>:<?php echo ($vo["server_port"]); ?></td>
                                        <td><?php echo (date('Y-m-d H:i:s', $vo["add_time"])); ?></td>
                                        <td><?php echo (get_camera_status($vo["status"])); ?></td>
                                        <td>
                                            <a title="编辑" href="<?php echo U('edit',array('id'=>$vo['id']));?>">编辑</a>
                                            <?php if($vo["status"] == 0): ?><a class="confirm ajax-get" title="禁用" href="<?php echo U('state',array('id'=>$vo['id'], 'status'=>0));?>">禁用</a>
                                                <a title="发布" href="<?php echo U('release',array('id'=>$vo['id'], 'zid'=>$vo['zone_id']));?>">发布</a>
                                            <?php else: ?>
                                                <a class="confirm ajax-get" title="启用" href="<?php echo U('state',array('id'=>$vo['id'], 'status'=>1));?>">启用</a><?php endif; ?>
