                    ...
                    <div class="operate-body">
                        ...
                        <div class="pull-right search-form form-inline">
                            <form action="" onsubmit="return false">
                                <input type="text" name="name" class="form-control search-input" value="<?php echo I('name');?>" placeholder="请输入方案名称">
                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td><?php echo ($row["plan_sn"]); ?></td>
                                    <td><?php echo ($row["plan_name"]); ?></td>
                                    <td><?php echo ($row["plan_price"]); ?></td>
                                    <td><?php echo ($row["area_surplus"]); ?></td>
                                    <td><?php echo ($row["income_expect"]); ?></td>
                                    <td><?php echo ($row["income_real"]); ?></td>
                                    <td><?php echo (date('Y-m-d',$row["plan_start"])); ?> 至 <?php echo (date('Y-m-d',$row["plan_end"])); ?></td>
                                    <td><?php echo (date('Y-m-d',$row["add_time"])); ?></td>
                                    <td>
                                        <?php switch($row['status']): case "0": ?><span class="label label-primary">未发布</span> 
                                                <a title="发布" class="confirm ajax-get" href="<?php echo U('dopush?id='.$row['plan_id']);?>">发布方案</a><?php break;?>
                                            <?php case "1": ?><span class="label label-success">进行中</span> 
                                                <a title="设置收益" href="javascript:doComplete('<?php echo ($row["plan_id"]); ?>','<?php echo ($row["income_expect"]); ?>')">设置收益</a><?php break;?>
                                        <?php default: ?>
                                            --<?php endswitch;?>
                                    </td>
                                    <td>
                                        <?php if($row["status"] < 1): ?><a title="编辑" href="<?php echo U('edit?id='.$row['plan_id']);?>">编辑</a><?php endif; ?>
                                        <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$row['plan_id']);?>">删除</a>
                                    </td>
            ...
            layer.use('extend/layer.ext.js', function(){
                layer.prompt({
                    ...
                    layer.confirm('用户将获得 <span class="label label-danger">'+cont+' 千克/平米</span> 的收益', {
                        ...
                        $.ajax({
                            url:'<?php echo U("Plan/docomplete");?>',
