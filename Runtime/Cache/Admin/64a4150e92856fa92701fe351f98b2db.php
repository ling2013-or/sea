                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td>
                                        <?php switch($row['status']): case "0": ?><span class="label label-primary">正常</span>
                                                <a title="禁用" class="confirm ajax-get" href="<?php echo U('dopush?id='.$row['id']).'&status=1';?>">禁用</a><?php break;?>
                                            <?php case "1": ?><span class="label label-primary">禁用</span>
                                                <a title="启用" class="confirm ajax-get" href="<?php echo U('dopush?id='.$row['id'].'&status=0');?>">启用</a><?php break;?>
                                        <?php default: ?>
                                            --<?php endswitch;?>
                                    </td>
                                    <td>
                                        <?php if($row["status"] == 0): ?><a title="编辑" href="<?php echo U('edit?id='.$row['id']);?>">编辑</a>
                                            <a title="发布" href="<?php echo U('release',array(id=>$row['id'],gid=>$row['goods_id']));?>">发布</a><?php endif; ?>
