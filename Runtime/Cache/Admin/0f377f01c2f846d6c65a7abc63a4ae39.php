                    ...
                    <div class="operate-body">
                        <div class="pull-right search-form form-inline">
                            <form action="" onsubmit="return false">
                                <input type="text" name="query" class="form-control search-input" value="<?php echo I('query');?>" placeholder="请输入ID/名称">
                                创建日期：
                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td><?php echo (get_pd_status($vo["type"])); ?></td>
                                        <td style="
                                            <?php if($vo["affect_money"] > 0): ?>color:red
                                            <?php else: ?>color:green<?php endif; ?>">
                                            <?php if($vo["affect_money"] > 0): ?>+<?php endif; ?>
                                            <?php echo ($vo["affect_money"]); ?>
                                        </td>
                                        <td><?php echo ($vo["available_money"]); ?></td>
...
//        $('#main-body',parent.document).height($('html').height());
    ...
    <script>
        window.setValue('type', '<?php echo I("type");?>');
        var start = {
            elem: '#start',
            format: 'YYYY-MM-DD',
