            ...
            <section class="panel">
                <div class="panel-body">
                    <div class="operate-body">
                        <div class="pull-left">
                            <a href="<?php echo U('addLevel');?>" class="btn btn-info">添加</a>
                        </div>
                        <div class="pull-right search-form form-inline">
                            <form action="" onsubmit="return false">
                                <input type="text" name="query" class="form-control search-input" value="<?php echo I('query');?>" placeholder="请输入等级ID/名称">
                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td><b class="<?php echo ($vo["icon"]); ?>"  ></b></td>
                                        <td><?php echo ($vo["cash_lower"]); ?></td>
                                        <td><?php echo ($vo["cash_highter"]); ?></td>
                                        <td><?php echo ($vo["true_name"]); ?></td>
                                        <td><?php echo (date('Y-m-d H:i:s', $vo["add_time"])); ?></td>
                                        <td><?php echo (date('Y-m-d H:i:s', $vo["update_time"])); ?></td>
                                        <td>
                                            <a href="<?php echo U('editLevel', array('id'=>$vo['level_id']));?>">编辑</a>
                                        </td>
