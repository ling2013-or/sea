                        ...
                        <div class="pull-right search-form form-inline">
                            <form action="" onsubmit="return false">
                                ...
                                <select name="status" class="form-control">
                                    ...
                                    <option value="0" <?php if((I('status') == 0) and (I('status') != '')): ?>selected<?php endif; ?>>未 处 理</option>
                                    <option value="1" <?php if(I('status') == 1): ?>selected<?php endif; ?>>处 理 中</option>
                                    <option value="2" <?php if(I('status') == 2): ?>selected<?php endif; ?>>已 处 理</option>
                                </select>
                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td><?php echo (get_feedback_status($vo["status"])); ?></td>
                                        <td>
                                            <?php if($vo["status"] == 0): ?><a class="confirm ajax-get" title="标记为处理中" href="<?php echo U('state',array('id'=>$vo['id'], 'status'=>1));?>">标记为处理中</a>
                                            <?php elseif($vo["status"] == 1): ?>
                                                <a class="confirm ajax-get" title="标记为已处理" href="<?php echo U('state',array('id'=>$vo['id'], 'status'=>2));?>">标记为已处理</a>
                                            <?php else: ?>
                                                - - -<?php endif; ?>
                                        </td>
