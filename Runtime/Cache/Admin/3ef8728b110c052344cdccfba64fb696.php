...
<section>
    ...
    <div class="page-heading">
        ...
        <ul class="breadcrumb">
            ...
            <?php if(empty($info)): ?><li class="active">
                    <a href="<?php echo U('Auth/index');?>">权限列表</a>
                </li><?php endif; ?>

            <?php if(!empty($info)): ?><li><a href="<?php echo U('Auth/index');?>">权限列表</a></li>
                <li class="active"><?php echo ($info["module_name"]); ?></li><?php endif; ?>
        </ul>
    </div>
                ...
                <div class="panel-body">
                    <div class="operate-body">
                        <div class="pull-left">
                            ...
                            <?php if(I('parent_id') != 0): ?><button class="btn a-back">返回</button><?php endif; ?>
                        </div>
                        <div class="pull-right search-form form-inline">
                            <form action="" onsubmit="return false">
                                <input type="text" name="name" class="form-control search-input" value="<?php echo I('name');?>" placeholder="请输入模块名称">
                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                        <td width="80"><?php echo ($row["module_index"]); ?></td>
                                        <td><i class="<?php echo ($row["module_img"]); ?>"></i></td>
                                        <td><?php echo ($row["module_name"]); ?></td>
                                        <td><a href="<?php echo U($row['module_bind']);?>" target="_blank"><?php echo ($row["module_bind"]); ?></a></td>
                                        <td>
                                            <a title="编辑" href="<?php echo U('edit?id='.$row['module_id']);?>">编辑</a>
                                            <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$row['module_id']);?>">删除</a>
                                            <?php if($row['is_menu'] == 1 && $row['parent_id'] == 0): ?><a title="删除" href="<?php echo U('index?parent_id='.$row['module_id']);?>">子菜单</a><?php endif; ?>
                                        </td>
