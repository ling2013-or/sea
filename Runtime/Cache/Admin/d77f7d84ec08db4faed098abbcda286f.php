                    ...
                    <div class="operate-body">
                        ...
                        <div class="pull-right search-form form-inline">
                            <form action="" onsubmit="return false">
                                <input type="text" name="name" class="form-control search-input" value="<?php echo I('name');?>" placeholder="请输入用户组名称">
                    ...
                    <section id="unseen">
                        <table class="table table-bordered table-striped table-condensed">
                            ...
                            <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td>
                                        <?php if($row['status'] == 0): ?><span class="text-danger">禁用</span>
                                            <?php else: ?>
                                            <span class="text-success">正常</span><?php endif; ?>
                                    </td>
                                    <td>
                                        <a title="查看" href="javascript:getMoreAdmin('<?php echo U('admin',array('id'=>$row['group_id']));?>')">管理员</a>
                                        <a title="编辑" href="<?php echo U('edit?id='.$row['group_id']);?>">编辑</a>
                                        <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$row['group_id']);?>">删除</a>
                                    </td>
